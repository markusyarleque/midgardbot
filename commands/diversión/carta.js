        ...
        if (args.length < 2) { // Si no se han dado todos los argumentos, envíar un mensaje de ayuda
            ...
            if(message.content.endsWith('-n'))
            {
                setTimeout(() => {
                    ...
                    target.user.send({embeds: [embcarta]}).catch(err => message.channel.send("El receptor no acepta mensajes directos, por favor, contáctalo, para que pueda escribirle <a:facherita:881034171571646464>"))
                ...
                setTimeout(() => {
                    ...
                    target.user.send({embeds: [embcartano]}).catch(err => message.channel.send("El receptor no acepta mensajes directos, por favor, contáctalo, para que pueda escribirle <a:facherita:881034171571646464>"))
