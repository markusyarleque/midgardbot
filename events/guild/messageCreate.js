...
module.exports = async (client, Discord, message) => {
    ...
    var mencionbot = message.content === '<@'+client.user.id+'>' || message.content.startsWith('midgard') || message.content.startsWith('mid') || message.content.startsWith('mg')
    console.log(mencionbot+' mencion: '+client.user.id)

    if(!message.content.startsWith(prefix) && !mencionbot) return; 
  
    const args = message.content.slice(prefix.length).trim().split(/ +/g) || message.content.slice(mencionbot).trim().split(/ +/g)
